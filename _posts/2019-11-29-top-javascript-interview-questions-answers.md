...
Number, String, Boolean, Object, Undefined etc
...
**What are global variables? How are these variable declared and what are the associated problems with using them?**
...
**Explain the working of timers in JavaScript? Also explain if there are any drawbacks of using the timer?**
...
"==" just examines for their value similarity in appearance but "===" is a more of a strict equality measure of the two variables which it returns false if either/both the value or type are different.
...
Cookies are generated when the user visits any website and that website stores some data they need about the user, these cookies which are typically small test files that are saved on a computer browser, could be descriptions of the user name and data about the shopping cart from previous visits, as an example.
...
They are Primitive and Reference types, where the former is a number or Boolean while the latter is string or date.
...
An array of three dimensional type.
